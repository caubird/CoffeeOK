...
{
    public partial class CoffeeOK_Control : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            {
                RobertComPort_comboBox.Items.Add(ComportNames[i]);
            }
            ...
            for (int i = 0; i < ComportNames.Count; i++)
            {
                ControlBoardComPort_comboBox.Items.Add(ComportNames[i]);
            }
        ...
        {
            //textBox1.Text = RobertConnectItem.Record;
            ...
            //if (step == 2)
            //{
            //    if (RobertConnectItem.WorkingConditionFlag == false)
            //    {
            //        RleasACup();
            //        MainTimer.Stop();
            //        Thread.Sleep(200);
            ...
            //    }
            //}
            if (RobertConnectItem.WorkingConditionFlag)
            {
                button6.BackColor = Color.Green;
                //
                textBox1.Text += "当前正在执行第" + step.ToString() + "步" + (char)0x0d + (char)0x0a;
                ...
                switch (step)
                {
                    case 1:
                        ...
                        DoMitionOne();
                        //step++;
                        textBox1.Text += "执行第1步" + (char)0x0d + (char)0x0a;
                        StepJumpControl();
                        //MessageBox.Show("执行第1步");
                        //DropCup_Flag = true;
                        break;
                    case 2:
                        //if (DropCup_Flag == true)
                        //{
                        //Thread.Sleep(500);
                    ...
                    case 3:
                        //button3_Click(null, null);
                        DoMitionThree();
                        textBox1.Text += "执行第三步" + (char)0x0d + (char)0x0a;
                        //step++;
                        StepJumpControl();
                        //MessageBox.Show("执行第2步");
                        break;
                    case 4:
                        ...
                        //step++;
                        StepJumpControl();
                        textBox1.Text += "执行第四步" + (char)0x0d + (char)0x0a;
                        //MessageBox.Show("执行第3步");
                        break;
                    case 5:
                        //button5_Click(null, null);
                        DoMitionFive();
                        StepJumpControl(); //step = 0;
                        textBox1.Text += "执行第五步" + (char)0x0d + (char)0x0a;

                        //MessageBox.Show("执行第4步");
                        break;
                    case 6:
                        ...
                        StepJumpControl();
                        textBox1.Text += "执行取货工步" + (char)0x0d + (char)0x0a;

                        //MessageBox.Show("执行取货工步");
                        break;
                    case 7:
                        button12_Click(null, null);
                        //step = 0;
                        StepJumpControl();
                        //MessageBox.Show("执行丢弃工步");
                        break;
                    default:
                        textBox1.Text += "任务结束，退出" + (char)0x0d + (char)0x0a;

                        Stop_TaskNow();
                        MainTimer.Stop();//执行完成，退出
                        break;
                }
            }
            else
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            //if (Responsebytes[0] == RobertConnectItem.Order_DoMission1[0] && RobertConnectItem.Order_DoMission1[1] == Responsebytes[1])
            //{
        ...
        private void ReadRobotCondition()
        {
            ...
            if (bytes[3] > 0 && bytes[0] == RobertConnectItem.ReadMissionMode[0] && bytes[1] == RobertConnectItem.ReadMissionMode[1])
            {
                RobertConnectItem.WorkingConditionFlag = true;
                button6.BackColor = Color.Green;
            }
            else if (bytes[3] == 0 && bytes[0] == RobertConnectItem.ReadMissionMode[0] && bytes[1] == RobertConnectItem.ReadMissionMode[1])
            {
        ...
        private void ConnectMysql_DB()
        {
            ...
            string sql = "select * from coffee_execute ";
        ...
        private void SetMysql_DB_TaskStatus(Task TaskItem)
        {
            ...
            MySqlCommand commn = new MySqlCommand("set names UTF8", conn);
            commn.ExecuteNonQuery();
            string sql = "update coffee_execute set status = '" + TaskItem.Status + "' where id= " + TaskItem.ID + "";
            //MySqlDataAdapter mda = new MySqlDataAdapter(sql, conn);
            //DataSet ds = new DataSet();
            // mda.Fill(ds, "coffee_excute");
            //mda.Fill(ds, "coffee_execute");
        ...
        private void InsertMysql_DB_TaskStatus(Task TaskItem)
        {
            ...
            commn.ExecuteNonQuery();
            string sql_insert =
                "INSERT into coffee_execute VALUES(" +
                TaskItem.ID + ", '" +
                TaskItem.CodeID + "', '" +
        ...
        {//
            // ConnectMysql_DB();
            //SetMysql_DB_TaskStatus(new Task("27","1",DateTime.Now.ToString () ,"0","","",""));
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            ...
            if (DB_InsertTimer.Enabled)
            {
                DB_InsertTimer.Stop();
            }
            else
            {
                DB_InsertTimer.Start();
            }
        }
    ...
    class Task
    {
        ...
        public Task(string id, string codeid, string creattime, string status, string Listid, string Priority, string ProcessID)
        {
            ID = id;
            CodeID = codeid;
            CreatTime = DateTime.Parse(creattime);
            Status = status;
            listid = Listid;
            priority = Priority;
            processid = ProcessID;
